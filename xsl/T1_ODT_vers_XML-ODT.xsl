...
<xsl:stylesheet exclude-result-prefixes="form fo svg text style office" version="1.0"
        ...
        <xsl:variable name="Balise">
            <xsl:choose>
                ...
                <xsl:when
                    ...
                    >mevital</xsl:when>
                <xsl:when
                    test="/office:document-content/office:automatic-styles/style:style[@style:name = $StyCarEnCours]/style:text-properties/@style:font-style-complex = 'italic'"/>
                <xsl:when
